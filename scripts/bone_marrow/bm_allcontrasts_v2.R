...
message("Samples: ", ncol(raw), " | miRNAs before filter: ", nrow(raw))
...
raw    <- raw[keep, ]
message("miRNAs after filter: ", nrow(raw))
...
message(sprintf("%-10s %-15s %8s %10s %10s",
...
for (nm in names(contrasts)) {
  ...
  out <- file.path(results_dir, paste0("BM_miRNA_", nm, "vsSed_perfectmatch.txt"))
