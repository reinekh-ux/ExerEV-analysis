...
out <- file.path(results_dir, "BM_miRNA_PooledExercise_vs_Sed_perfectmatch.txt")
...
message("\n=== BONE MARROW POOLED — PERFECT MATCH ===")
message("Total tested: ", nrow(res))
message("padj < 0.05: ", sig_total)
message("padj < 0.05 AND baseMean >= 10: ", sig_real)
...
for (target in targets) {
  ...
  r <- res[hit[1],]
  message(sprintf("%-15s | baseMean %6.1f | log2FC %+.3f | pvalue %.4f | padj %.4f",
