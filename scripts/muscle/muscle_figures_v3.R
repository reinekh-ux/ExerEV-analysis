...
# ── COLLECT ALL SIGNIFICANT miRNAs ACROSS ALL TIMEPOINTS ───────────────────
message("Collecting all significant hits across timepoints...")
all_sig <- c()
res_list <- list()
...
for (contrast in contrasts) {
  ...
  sig <- rownames(res)[!is.na(res$padj) & res$padj < 0.05 &
                         res$baseMean >= min_bm]
  sig_names <- res$short_name[rownames(res) %in% sig]
  all_sig <- union(all_sig, sig_names)
}

message("Total unique significant miRNAs (baseMean>=10, padj<0.05): ",
        length(all_sig))
message(paste(sort(all_sig), collapse=", "))

# ── FIGURE 1: VOLCANO PLOTS — fix overlaps ─────────────────────────────────
...
for (contrast in contrasts) {
  ...
  res$col[!is.na(res$padj) & res$padj < 0.05 &
            ...
            res$baseMean >= min_bm] <- col_up
  res$col[!is.na(res$padj) & res$padj < 0.05 &
            res$log2FoldChange < 0 &
            res$baseMean >= min_bm] <- col_dn
...
png(file.path(fig_dir, "Fig1_Volcano_allTimepoints_v3.png"),
...
# ── FIGURE 2: TRAJECTORY PLOTS FOR ALL SIGNIFICANT miRNAs ─────────────────
message("\nMaking trajectory plots for ", length(all_sig), " miRNAs...")

# build full trajectory data
traj_list <- list()
for (contrast in contrasts) {
  res <- res_list[[contrast]]
...
# determine direction per miRNA (majority direction across significant timepoints)
...
# arrange in grid — ncol=4 for many miRNAs
n_mirnas <- length(traj_plots)
ncols    <- 4
nrows    <- ceiling(n_mirnas / ncols)
plot_h   <- max(10, nrows * 3.5)

png(file.path(fig_dir, "Fig2_Trajectory_allSigmiRNAs_v3.png"),
    width=16, height=plot_h, units="in", res=300)
grid.arrange(grobs=traj_plots, ncol=ncols,
             top="Temporal log2FC trajectory — all significant muscle sEV miRNAs (baseMean>=10, padj<0.05)")
dev.off()
message("Trajectory plots saved — ", n_mirnas, " miRNAs in ", ncols,
        " columns x ", nrows, " rows")

# ── FIGURE 3: HEATMAP — all significant miRNAs ────────────────────────────
message("\nMaking heatmap...")
...
# cluster miRNAs by direction and significance pattern
...
heatmap_h <- max(5, length(all_sig) * 0.35 + 2)
ggsave(file.path(fig_dir, "Fig3_Heatmap_allSigmiRNAs_v3.png"),
       ph, width=8, height=heatmap_h, dpi=300)
message("Heatmap saved")

message("\nAll v3 figures in: ", fig_dir)
message("=== DONE ===")
